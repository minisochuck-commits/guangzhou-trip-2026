...
//
// 两批素材合在这里，谁也不用手改：
//   lib/image-credits.ts  —— 老脚本 scripts/pick-images.py 生成的第一批
//   lib/guide-photos.ts   —— scripts/build-guide-photos.mjs 生成的第二批（自带 alt 与图说）
...
/** 第二批照片知道自己的像素尺寸：写进 <img width height>，滚动时不跳版。 */
const SIZES: Record<string, { width: number; height: number }> = Object.fromEntries(
  GUIDE_PHOTOS.map((photo) => [photo.key, { width: photo.width, height: photo.height }]),
);
...
/** 第二批照片自带的三语 alt 与图说。第一批的说明写在各自的数据条目里。 */
const TEXTS: Record<string, { alt: L10n; caption: L10n }> = Object.fromEntries(
  GUIDE_PHOTOS.map((photo) => [photo.key, { alt: photo.alt, caption: photo.caption }]),
);
