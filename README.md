...
- **只有一个纵向滚动条**：三列在 390px 上全放得下，**表格不横向滚动**。
  ...
  日期条 + 列头一起 sticky 在固定栈下面（页头 + 标签条的高度由 `ResizeObserver`
  量出来写进 `--trip-header-h`）；可见列头是表格外面那条 bar，和 `<colgroup>` 读同一组变量，
...
顶部是**一个固定栈**（整页只有这一处 `sticky top-0`）：印章 + 标题 + 日期区间一行，
**人员下拉**（默认 Reham，点开才有其他人和「全部」）与语言切换一行，
最下面是**两个标签**（全部行程 / 来华指南）。三部分都在同一个 sticky 容器里，
所以**滚到哪里都能切换标签**；`Tabs` 根包住页头与 `<main>`，面板仍在 `<main>` 里，
标签语义与默认「全部行程」不变。
固定栈的高度由 `ResizeObserver` 量出来写进 `--trip-header-h`，日期条 + 列头
按它停在正下方，指南的章节锚点也按它算 `scroll-margin-top`。
下拉 44px / 16px、语言按钮 44px、标签 44px 的触控尺寸不动，
**压缩的只是纵向留白**（页头两行与标签上下）。
...
| 来华指南 | 顶上是**默认可见的欢迎开篇**（珠江城市全景 `skyline.jpg`，按 1200×492 自然比例显示、不裁建筑；加「欢迎来到广州」和两句话，不折叠），下面**按客人到达的顺序**读（owner 2026-09-12 定）：出发前准备（机场中文地址与两家航司行李额收在本章的折叠详情里）→ 酒店与联系 → 琶洲：你住的这块地 → 广州与你们 → 认识今天的广州 → 名创优品在广州（品牌从广州出发 / 今天的总部 / 正佳的 MINISO LAND 广州壹号店）→ 科技就在身边 → 礼拜与清真餐 → 食在广州（含 14 个特色餐饮品牌推荐）→ 当地生活习惯 → 广州商圈 → 半日路线 → 中文短句 → 资料与图片出处。一章一张卡，默认全部收起，点开整章连着读完。**没有分组标题、没有勾选清单、没有目录式导航** |
...
- 9/21 抵达报到。
- 9/22–9/24：Li/Qiuting、Rahma 参会（9/24 09:00–18:00参加订货会）。结束后 Li/Qiuting
...
- Ahmed / Mohamed：9/22–9/23 参会；9/24–9/25 由 **MINISO 广州区域负责人**带领巡店学习，
  **课表、路线、门店、接送全部待中国团队确认** —— 不写固定驻某一家店，也没有固定上下课时间；
  9/26 自由活动。
- Reham：9/22 上午参会、下午自由；**9/24 13:50–17:30参加国际英语巡店**，9/23全天自由活动；其余时段自由，9/25–9/26 自由，9/27 离境。
...
- **Reham 9/24 国际英语巡店交通由 MINISO 统一安排**，集合时间地点待 Rahma 确认。仍然待定的外出交通：
